...
describe('SearchPanel', () => {
  ...
  it('should render input and button', () => {
    render(<SearchPanel onSearch={mockOnSearch} searchTerm="" />);
  ...
  it('should save trimmed query to localStorage when button is clicked', async () => {
    ...
    expect(localStorage.getItem('searchTerm')).toBe(JSON.stringify('test'));
  });
  ...
  it('should update localStorage with new search query', async () => {
    ...
    expect(localStorage.getItem('searchTerm')).toBe(JSON.stringify('first'));

    await user.clear(input);
    await user.type(input, 'second');
    await user.click(button);

    expect(localStorage.getItem('searchTerm')).toBe(JSON.stringify('second'));
  });
});
